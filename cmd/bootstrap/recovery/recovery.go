// Package recovery decides how a node that is (re)joining replication should
// catch up with the primary: by replicating the transactions it is missing, or
// by throwing its data away and cloning.
//
// The decision is made from GTID sets alone, so it is the same for group
// replication and for async replication; only the caller differs in what it
...
// SQLRunner is what the decision needs from a server. *Runner implements it
// over database/sql; tests implement it directly.
...
// CompareGTIDs relates set a to set b: a CONTAINS b, a is CONTAINED in b,
// EQUAL, DISJOINT, or INTERSECTS (each side has transactions the other lacks).
// The set arithmetic runs on the server behind shell.
...
// CheckReplicaState classifies the replica against the primary:
//
//   - New: the replica has nothing and the primary has everything since the
//     beginning; a clone is the only way to load it.
//   - Identical: nothing to do.
//   - Recoverable: the replica is behind and the primary still has the binlogs
//     it needs; replication will catch it up.
//   - Irrecoverable: the replica is behind but the primary purged the binlogs
//     in between; only a clone can close the gap.
//   - Diverged: the replica has transactions the primary does not. Nothing
//     can replicate those away; keeping them means two histories. A
//     demoted primary that took writes after the failover looks like this.
...
// Method picks clone or incremental for a group replication member. Beyond the
// GTID state it honours group_replication_clone_threshold, which group
// replication itself applies when it chooses distributed recovery.
func Method(ctx context.Context, primary, replica SQLRunner) (innodbcluster.RecoveryMethod, error) {
	...
	case innodbcluster.ReplicaGtidRecoverable, innodbcluster.ReplicaGtidIdentical, innodbcluster.ReplicaGtidNew:
		if exceeded, err := CloneThresholdExceeded(ctx, primary, replica); err != nil {
...
// Runner is SQLRunner over an open connection.
type Runner struct {
	DB *sql.DB
}

func (r *Runner) GetGTIDExecuted(ctx context.Context) (string, error) {
	var gtid string
...
func (r *Runner) GetGTIDPurged(ctx context.Context) (string, error) {
	var gtid string
...
func (r *Runner) GTIDSubtract(ctx context.Context, set, subset string) (string, error) {
...
func (r *Runner) GTIDSubtractIntersection(ctx context.Context, a, b string) (string, error) {
	var sub sql.NullString
...
func (r *Runner) IsPurgedSubsetOfExecuted(ctx context.Context, purged, executed string) (bool, error) {
	var isSubset bool
...
func (r *Runner) GetCloneThreshold(ctx context.Context) (uint64, error) {
