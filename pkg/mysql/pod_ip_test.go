...
// runPodIP calls pod_ip from build/lib/util.sh with `hostname -I` stubbed to
// print addrs, and returns what it printed. env is appended to the process
// environment, so a POD_IP_*_REGEX given there reaches the function.
func runPodIP(t *testing.T, addrs string, env ...string) string {
	...
	cmd := exec.CommandContext(t.Context(), "bash", "-c", "set -eo pipefail; . "+lib+"; pod_ip")
